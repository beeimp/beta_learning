...
-- 12-34
%sql
SELECT DATE_FORMAT(issue_date, '%Y/%m') months,
       SUM(cases) case_num,
       SUM(deaths) death_num
  FROM covid19_data
 WHERE countrycode = 'KOR'
...
-- 12-38
%sql 
SELECT sibsp + parch AS family_cnt,
       survived, 
       COUNT(*) AS cnt
  FROM titanic
 GROUP BY 1, 2
...
SELECT gubun, 
       SUM(CASE WHEN months = 202001 THEN 
                  CASE WHEN gubun = '1.확진' THEN case_num 
                       ELSE death_num END
                ELSE 0 END ) "202001",
       SUM(CASE WHEN months = 202002 THEN 
                  CASE WHEN gubun = '1.확진' THEN case_num 
                       ELSE death_num END
                ELSE 0 END ) "202002",
       SUM(CASE WHEN months = 202003 THEN 
                  CASE WHEN gubun = '1.확진' THEN case_num 
                       ELSE death_num END
                ELSE 0 END ) "202003"                           
  FROM raw_data3    
